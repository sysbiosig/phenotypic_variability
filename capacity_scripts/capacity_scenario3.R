...
gamma.capacity.continuous <- data.frame()
set.seed(122)
for(stim.type.chosen in chosen.stimulants){
  ...
  gamma.capacity.continuous <- rbind(gamma.capacity.continuous,
                          data.frame(stim_type = stim.type.chosen,
                                     gamma.capacity = CM.capacity$capacity.continuous)) 
}
# gamma.capacity.continuous1 <- gamma.capacity.continuous

print(gamma.capacity.continuous)
